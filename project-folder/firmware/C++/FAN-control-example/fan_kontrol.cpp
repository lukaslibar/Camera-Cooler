...
constexpr uint FEN_EN = 6;
constexpr uint8_t DUTY_CYCLE = 30;
...
int main(){
    ...
    uint8_t DUTY_CYCLE_T = 100 - DUTY_CYCLE;
    uint wrap = ura / FREQ - 1;
    uint level = (DUTY_CYCLE_T * (wrap + 1)) / 100;

    pwm_set_clkdiv(slice_st, 1.0f); // brez deljenja ure
    pwm_set_wrap(slice_st, wrap);
    pwm_set_chan_level(slice_st, kanal, level);

    gpio_set_dir(FEN_EN, GPIO_IN); // GPIO mora biti nastavljen na vhod da lahko deluje v HiZ.

    pwm_set_enabled(slice_st, true);

    gpio_set_pulls(FEN_EN, false, false); // prižgan
